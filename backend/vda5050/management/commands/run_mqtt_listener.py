...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            self.stdout.write(f'Connecting to {mqtt_broker}:{mqtt_port} as {client_id}...')
        ...
        try:
            ...
            if state.errors:
                for err in state.errors:
                    ...
                    if err.get('errorType') in ['orderError', 'validationError', 'noRouteError']:
                        if order.status != Order.OrderStatus.REJECTED:
                            order.status = Order.OrderStatus.REJECTED
                            order.rejection_reason = f"{err.get('errorType')}: {err.get('errorDescription')}"
                            order.save()
                            logger.warning(f"Order {order.order_id} REJECTED by AGV: {order.rejection_reason}")
            ...
            if order.nodes: # Make sure order has nodes
                ...
                if (state.last_node_id == last_node_in_order and 
                    not state.driving):
    ...
    def handle_connection(self, manufacturer, serial_number, payload):
        try:
            agv = AGV.objects.get(manufacturer=manufacturer, serial_number=serial_number)
    ...
    def publish_order(self, order):
        ...
        try:
            ...
            order.status = 'SENT'
            order.save()
            logger.info(f"Dispatched Queued Order {order.order_id} to {agv.serial_number}")
        except Exception as e:
