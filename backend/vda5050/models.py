...
class Order(models.Model):
    ...
    order_id = models.CharField(max_length=100, unique=True, help_text="Unique order ID from VDA")
    order_update_id = models.IntegerField(default=0, help_text="Used for updating/extending orders")
    ...
    rejection_reason = models.TextField(blank=True, null=True, help_text="Reason why the AGV rejected this order")
...
class AGVState(models.Model):
    ...
    received_at = models.DateTimeField(auto_now_add=True, help_text="Time when the Server received it")
    ...
    battery_state = models.JSONField(default=dict, help_text="{charge, voltage, health...}")
    agv_position = models.JSONField(default=dict, null=True, help_text="{x, y, theta, mapId...}")
    ...
    information = models.JSONField(default=dict, null=True, help_text="Other debug information")
    class Meta:
...
class InstantAction(models.Model):
    ...
    agv = models.ForeignKey(AGV, on_delete=models.CASCADE, related_name="instant_actions")
    header_id = models.IntegerField(default=0)
    timestamp = models.DateTimeField(auto_now_add=True)
    
    # Action Type using VDA5050 standard
    action_type = models.CharField(max_length=50, choices=ACTION_CHOICES)
    action_id = models.CharField(max_length=100, unique=True, help_text="Unique action ID")
...
class GraphEdge(models.Model):
    ...
    start_node = models.ForeignKey(GraphNode, related_name='outgoing_edges', on_delete=models.CASCADE)
    end_node = models.ForeignKey(GraphNode, related_name='incoming_edges', on_delete=models.CASCADE)
    map_id = models.CharField(max_length=100, default="map_1")
