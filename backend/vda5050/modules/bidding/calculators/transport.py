...
from ...constant import (
    C_BASE, C_LOAD_COEFF, AGV_SPEED_MPS,
...
class TransportCalculator:
    ...
    def __init__(self, speed_mps=AGV_SPEED_MPS, c_base=C_BASE, c_load_coeff=C_LOAD_COEFF):
        """
        ...
        logger.debug(f"TransportCalculator initialized: speed={speed_mps}m/s, "
    ...
    def calculate_travel_time(self, distance_m):
        ...
        logger.debug(f"Travel time: {distance_m}m @ {self.speed_mps}m/s = {time_s:.2f}s")
        return time_s
    ...
    def calculate_energy_consumption(self, distance_m, load_kg=0):
        ...
        logger.debug(f"Energy consumption: ({self.c_base} + {self.c_load_coeff}*{load_kg}kg) * {distance_m}m = {energy_kj:.2f}kJ")
        return energy_kj
    ...
    def validate_metrics(self, energy_kj, time_s):
        ...
        if energy_kj <= 0 or time_s <= 0:
            logger.warning(f"Invalid metrics detected (E={energy_kj}, T={time_s}). "
