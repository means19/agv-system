...
class AGVTestRunner:
    ...
    def monitor_completion(self, timeout=300):
        ...
        while len(completed_orders) < len(self.tasks_created):
            ...
            print(f"  Progress: {len(completed_orders)}/{len(self.tasks_created)} "
                  f"({progress:.1f}%) - Elapsed: {elapsed:.1f}s", end="\r")
    ...
    def wait_for_completion(self):
        ...
        while wait_time < estimated_time:
            ...
            print(f"   Progress: {min(progress, 100):.1f}% ({wait_time}s / {estimated_time:.0f}s)", end="\r")
    ...
    def evaluate_actual_results(self):
        ...
        try:
            ...
            all_agvs = set(list(self.agv_task_count.keys()) + list(actual_agv_distribution.keys()))
                    ...
                    if response.status_code == 200:
                        ...
                        if states_response.status_code == 200:
                            ...
                            if states:
                                ...
                                battery = latest_state.get('battery_state', {}).get('batteryCharge', 0)
                                tasks = actual_agv_distribution.get(agv_serial, 0)
                                battery_icon = "🔋" if battery > 50 else "🪫" if battery > 20 else "⚠️ "
                                print(f"   {battery_icon} {agv_serial}: {battery:.1f}% ({tasks} tasks)")
                except:
                    pass
    ...
    def generate_report(self):
        ...
        if self.start_time and self.end_time:
            ...
            print(f"   Start: {datetime.fromtimestamp(self.start_time).strftime('%H:%M:%S')}")
            print(f"   End: {datetime.fromtimestamp(self.end_time).strftime('%H:%M:%S')}")
        ...
        if self.start_time and self.end_time:
            ...
            if total_time > 0:
                ...
                print(f"   Avg time per task: {total_time/len(self.tasks_created):.2f} seconds")
