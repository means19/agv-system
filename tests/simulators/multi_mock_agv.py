...
class MockAGV:
    ...
    def __init__(self, serial_number: str, initial_config: Dict):
        ...
        # MQTT setup
        self.client = mqtt.Client(client_id=f"mock_{serial_number}_{uuid.uuid4().hex[:4]}")
        self.client.on_connect = self._on_connect
        self.client.on_message = self._on_message
        
        # Topics
        self.topic_connection = f"uagv/v2/{self.manufacturer}/{self.serial_number}/connection"
        ...
        self.topic_instant_action = f"uagv/v2/{self.manufacturer}/{self.serial_number}/instantActions"
        ...
        print(f"🤖 Initialized {self.serial_number} at {self.state['lastNodeId']} (Battery: {self.state['battery']}%)")
    ...
    def _handle_order(self, order: Dict):
        ...
        if len(nodes) > 2:
            self.state["current_load"] = {"type": "package", "weight": 10}  # Placeholder
    ...
    def _send_connection_message(self, status: str):
        ...
        self.client.publish(self.topic_connection, json.dumps(payload), qos=1, retain=True)
    ...
    def _simulate_movement(self):
        ...
        if not self.state["nodes_to_visit"]:
            ...
            self.state["driving"] = False
            self.state["current_load"] = None
            print(f"🏁 {self.serial_number}: Reached destination at {self.state['lastNodeId']}")
            return
        ...
        print(f"🚚 {self.serial_number}: Arrived at {node_id} (Battery: {self.state['battery']:.1f}%)")
...
class MultiAGVSimulator:
    ...
    def print_status(self):
        ...
        for agv in self.agvs:
            ...
            print(f"  {agv.serial_number}: {status} | {load} | "
